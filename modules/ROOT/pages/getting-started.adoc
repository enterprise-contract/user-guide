...
= Getting started with Enterprise Contract and Red Hat Trusted Application Pipeline

In this guide I'll create a new application, create an integration test to run
the Enterprise Contract, examine its results, and then customize the default
configuration.

I'm working with a real example application on the current (March 28, 2023)
"beta" version of RHTAP.
...
NOTE: I presume in the near future the url will be changed so it's no longer
`stonesoup` and also the title "CI/CD" might be changed to Red Hat Trusted
Application Pipeline or RHTAP.
...
For this example, I'll use my own fork of the node sample app, specifically
link:https://github.com/simonbaird/devfile-sample[this one]. I left the
application name as the default `my-app`.
...
I chose "Custom build", and reached this screen.
...
NOTE: All of the above is a pretty standard guide to creating an app, so it
might be already covered by other documentation. Potentially it could be
removed from this page entirely and this particular guide could start from the
next section.
...
Pipeline to run:: `enterprise-contract`. Must be exactly correct, since this is the name of the pipeline definition in the tekton bundle.
...
- I presume you could also use a container image digest to pin it more securely.
====
...
that will be the preferred way to do it. I'll update these docs to demonstrate
...
wait for the fix to be merged, I'll use the opportunite to demonstrate
customizing the Enterprise Contract configuration to skip the known false
positive.
...
Let's take a closer look at the EC contract output and the configuration used.
...
need to do this repeatedly. Would it be possible to adjust timeout longer,
perhaps an hour instead of whatever it is now? (It feels like five or ten
minutes.) There is a
...
Here's some commands to extract the Enterprise Contract output. (This works
conveniently if the integration test is the most recent PipelineRun, but it
should be easy enough to adapt the commands as required if that is not the
case.)
...
NOTE: This works but it's a little clunky. Is there an easier way to access the
TaskRun log?

NOTE: I thought it would be possible to use `tkn` here, but I get an error,
...
The EC output is YAML so you can select pieces of it using the yq command. For example, this produces a list of the violations:
...
To create a custom configuration for EC, it's useful to extract the current (default) configuration.
...
For verifying signatures the public key might be useful. We can extract that
from the output also like this:
...
Let's also look at the Integration Test Scenario. This is the record that
corresponds to the integration test visible under the "Integration Tests". In
my case it's called `check-contract`.
...
`pipeline` which were initially entered via the UI. Later we'll be modifying this
...
We'll initially use the same configuration as the default ECP.

Create a yaml file call `ecp.yml` with the following content:
...
Let's edit the new `my-ec-policy.yml` file. I'll modify the description and add
an `exclude` option designed to skip all the `cve` rules. After editing the
`my-ec-policy.yml` file looks like this:
...
We now have a customized ECP record in our namespace. The next step is to
modify the IntegrationTestScenario record to tell it to use that configuration.
...
NOTE: It would be nice if this could be done via the web UI. Perhaps there
could be a general way to modify the params for an IntegrationTestScenario.
...
NOTE: I get a warning using kubectl apply like this, since the original CR doesn't have the expected annotations. Is there a more
standard way to patch a CR that would avoid the warning. I guess `kubectl edit` would work here, but it's harder to document.
...
At this point we should be ready to re-run the Integration Test.

== Running the Integration Test again with the customized EC policy

To re-run the Integration Test we need to trigger a build. As before, we'll do
that by making a commit in the source repo. To save time I'm again commiting
directly to main branch via the GitHub web UI. The commit is
link:https://github.com/simonbaird/devfile-sample/commit/2edbaceb46dfa0ca396017d2b1000acfffe7f68e[here].

The commit triggers a new build. Notice the "Tests" is still marked as failing.
...
Let's confirm that it's working as we expected. Firstly, if we view the
...
[NOTE]
====
- "All rules" here means effectively all the rules defined in the
  `quay.io/hacbs-contract/ec-release-policy` bundle which is defined under `sources`.
  Those rules are described link:https://enterprise-contract.github.io/ec-policies/release_policy.html[here].
  You can also write your own custom rules but we don't have good documentation for that yet.
  If you want some pointers, feel free to ask questions on the `#stonesoup-users` Red Hat slack channel.
- Setting `configuration` to just `{}` would also work to apply all the rules, but I think using
  an include with a wildcard helps express the intention better.
====

=== Triggering another build

Notice there's no need to modify the IntegrationTestScenario record now, since it's already pointing to our customized ECP CR.

Trigger a new build by making a bogus commit once again.
...
[NOTE]
====
- Describing how to debug and fix all these EC violations is left for another day.
- Of all the errors listed above, the "unacceptable task bundle" seems the most unintuitive, so we may need some specific documentation describing acceptable bundle concepts.
====
